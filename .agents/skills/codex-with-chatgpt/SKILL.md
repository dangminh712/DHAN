...
description: >
  ...
  "ä½¿ç”¨ Codex with ChatGPT ..." / "Set up Codex with ChatGPT" / "ç”¨ ChatGPT è§„åˆ’",
...
control messages between you and ChatGPT stay tiny (< 1 KB) â€” ChatGPT pulls
...
2. NEVER show the user technical internals (MCP, OAuth, PKCE, tunnel, ports, localhost).
   Speak in terms of "è¿žæŽ¥ ChatGPT / å®‰å…¨è¿žæŽ¥ / é…å¯¹". The only exception is the
...
4. If something fails, run `c2c doctor` and repair silently. Only involve the user
   ...
   ChatGPT setup** below â€” and then give them ONE action.
   ...
   - `setupMode` missing: tell the user exactly `setupChoicePrompt`, wait for
     ã€Œ1ã€orã€Œ2ã€, then `c2c prefs set --setup-mode auto|manual --json`.
...
5. ALWAYS use the built-in in-app browser (iab) for every ChatGPT step.
   ...
   (Chrome, Safari, Edgeâ€¦), and never use `open <url>` to hand off to one.
   - The ONLY exception: the user explicitly says the Cloudflare login must use
     their own browser session â€” that single Cloudflare login step may go through
   ...
   - If the user asks to run ChatGPT in their own browser, refuse politely and
     explain: "Codex éœ€è¦æŒç»­è°ƒç”¨ ChatGPT å’Œé…ç½®è¿žæŽ¥ï¼Œè¿™ä¼šé¢‘ç¹æ“ä½œé¡µé¢ï¼Œå¯èƒ½å½±å“
     ä½ æµè§ˆå™¨çš„æ­£å¸¸ä½¿ç”¨ã€‚ChatGPT åªèƒ½è·‘åœ¨å†…ç½®æµè§ˆå™¨é‡Œã€‚" Only if the user replies
     with an explicit "æˆ‘æ„¿æ„æ‰¿æ‹…å½±å“" may you proceed in their browser; otherwise
     keep ChatGPT in the built-in browser, every time they ask.
6. Conversation reuse depends on `c2c session --json` â†’ `conversation.mode`
   ...
   - **project** (new workspaces, or an existing workspace that opted in):
     ...
     conversation opens a new chat from the Project collection page â€” never
   ...
   own connectors â€” never edit those.
...
9. **Doctor gate.** After `c2c doctor --json`, do not `goto` ChatGPT and do not
   send `[C2C]` until local is green â€” except the reconnect settings pages when
   ...
   - `namedRepair.needed` is true (user must log in to Cloudflare, then doctor again.
     Do not Delete the ChatGPT connector â€” the address did not change)
   - `report.bridge` says çŠ¶æ€æ— æ³•ç¡®è®¤: the local bridge may still be running.
...
2. **One tab.** Create the ChatGPT tab once (`tabs.new()`). After that, only
   `tab.goto(...)` to switch URLs. If the tab still exists, claim it â€” never
...
3. **Foreground + keep (standby).** Right after opening or claiming the tab:
   - `await (await iab.capabilities.get("visibility")).set(true)` â€” first-time
...
4. **URLs only** (same tab, `goto` â€” never hunt menus):
   - å¼€å‘äººå‘˜æ¨¡å¼: `https://chatgpt.com/#settings/Security`
     (skip when `c2c prefs --json` has `developerModeEnabled: true`)
   - æ’ä»¶æ€»ç®¡: `https://chatgpt.com/plugins`
   - åŠ æ’ä»¶: `https://chatgpt.com/plugins#settings/Connectors?create-connector=true&redirectAfter=%2Fplugins`
   - æ–°å¯¹è¯ (long-chat only, and only if no saved chat): `https://chatgpt.com/`
   ...
   - Saved Project collection: `conversation.projectUrl`
     (`https://chatgpt.com/g/g-p-â€¦/project`)
   ...
   via the åŠ æ’ä»¶ URL (same name, new Server URL). Do not put that public
   address into Project instructions â€” write the connector **name** only.
...
7. **One conversation, Chat mode.** The first ChatGPT chat is the C2C
   conversation. Chat and Work (èŠå¤© / å·¥ä½œ) are separate: a Work conversation
   ...
   prompt. If it is Work, do not continue there â€” Switch to a new Chat
   ...
   A collection or chat page that shows only `Retry` / `é‡è¯•` is a navigation
   ...
   click the on-page `Open â€¦ project` / `æ‰“å¼€â€œâ€¦ â€é¡¹ç›®` link â€” that same-site
...
8. **Wait for a ChatGPT reply (do not hold one long browser wait).** After you
   ...
   and do not screenshot-poll. Every 20â€“30 seconds, one cheap DOM check:
   - still generating â†’ wait again (do not type, do not resend);
   - `STATE: PLAN` / `DONE` / `BLOCKED` / the verify workspace name â†’ read it
     and continue the existing protocol;
   - visible error â†’ repair; do not start a new chat.
...
- The codex-with-chatgpt checkout lives at: `C:\Users\DangMinh\codex-with-chatgpt`
...
2. `c2c sandbox-allow --json` (do not pass `-w`) â€” writes the C2C state directory into Codex's
...
- `{ "updateAvailable": false }` â†’ continue silently. Never mention the check.
- `{ "updateAvailable": true }` â†’ tell the user one line:
  "æ£€æµ‹åˆ° Codex with ChatGPT æœ‰æ–°ç‰ˆæœ¬ï¼Œæˆ‘å…ˆæ›´æ–°ä¸€ä¸‹ï¼ˆçº¦ 1 åˆ†é’Ÿï¼‰ï¼ŒéšåŽç»§ç»­ä½ çš„ä»»åŠ¡ã€‚"
  Then run the update workflow below, and CONTINUE the original task afterwards.

## Workflow: updateï¼ˆ"æ›´æ–° Codex with ChatGPT"ï¼Œor triggered by the daily checkï¼‰
...
5. Tell the user "âœ“ å·²æ›´æ–°åˆ°æœ€æ–°ç‰ˆæœ¬" â€” then resume whatever task triggered this.
...
Speak only of ä¸´æ—¶åœ°å€ / å›ºå®šåŸŸå / ç™»å½• Cloudflare.
...
3. If `needsChoice` is true: tell the user exactly `userPrompt` and wait.
   - æ²¡æœ‰è´¦å· / æ²¡æœ‰åŸŸå / ä¸´æ—¶ / ä¸ç”¨ â†’
     `c2c tunnel choose -w <ws> --mode quick --json`
   - æœ‰åŸŸåï¼ˆä¾‹å¦‚ example.comï¼‰â†’ first tell them `loginPrompt`, then
...
## Workflow: first-time setupï¼ˆ"ä½¿ç”¨ Codex with ChatGPT å®Œæˆé¦–æ¬¡é…ç½®"ï¼‰
...
3. Run `c2c sandbox-allow --json`, then **Connection choice**, then
   ...
   `sandbox-allow` edits Codex `config.toml` only â€” it adds C2C's state directory
   ...
   â†’ returns `{ mcpUrl, pairingCode, workspaceName, connectorName, ... }`.
   `connectorName` is this workspace's plugin title (legacy installs stay
   `Codex with ChatGPT`; additional workspaces get `Codex with ChatGPT Â· <name>`).
...
4. `c2c prefs --json` (this machine, not this workspace).
   - If `setupMode` is null: tell the user exactly `setupChoicePrompt`. Wait
     forã€Œ1ã€orã€Œ2ã€. Then `c2c prefs set --setup-mode auto` or `--setup-mode manual`.
   ...
   - `setupMode: "manual"`: skip step 5's automatic ChatGPT settings. Go to
     **Guided manual ChatGPT setup** (chosen). Opening line:
     `æŽ¥ä¸‹æ¥ç”¨æ‰‹åŠ¨æ•™å­¦é…ç½®ã€‚ä¸€æ¬¡åªéœ€è¦åšä¸€ä¸ªæ“ä½œã€‚`
     Do not say è‡ªåŠ¨é…ç½®æ²¡æœ‰æˆåŠŸ.
...
5. Open ChatGPT on the ONE iab tab (see **In-app browser**). Foreground +
   ...
   - å¼€å‘äººå‘˜æ¨¡å¼: skip `https://chatgpt.com/#settings/Security` when
     `developerModeEnabled` is true. Otherwise open it, enable å¼€å‘äººå‘˜æ¨¡å¼
     ...
     and retry create â€” do not skip that recovery.
   - å·²æœ‰è¯¥ `connectorName`: `https://chatgpt.com/plugins` â€” Delete it (never
     Reconnect). Then `goto` the åŠ æ’ä»¶ URL below.
   - è¿˜æ²¡æœ‰ / åˆšåˆ æŽ‰: `https://chatgpt.com/plugins#settings/Connectors?create-connector=true&redirectAfter=%2Fplugins`
     ...
     Connected / authorized / pairing accepted, continue â€” do NOT wait for 8
     tools on this page.
6. Same tab: open the first C2C chat per **Conversation management**
   ...
   in long-chat). Confirm Chat mode per **In-app browser** Â§7 (if it is Work,
   open a new Chat conversation instead). Send the boot prompt from
   `docs/protocol.md` Â§Boot Prompt, then (same chat) send:
   `Use the "<connectorName>" connector: call workspace_info and read hello-style top-level file. Reply with the workspace name.`
   Confirm the reply matches `workspaceName` (wait per **In-app browser** Â§8).
...
to do ("è¯·ç™»å½• ChatGPTï¼Œå®ŒæˆåŽå‘Šè¯‰æˆ‘'å¥½äº†'"), then continue.
...
- Chosen (`setupMode: "manual"`): `æŽ¥ä¸‹æ¥ç”¨æ‰‹åŠ¨æ•™å­¦é…ç½®ã€‚ä¸€æ¬¡åªéœ€è¦åšä¸€ä¸ªæ“ä½œã€‚`
- Failure fallback: `è‡ªåŠ¨é…ç½®æ²¡æœ‰æˆåŠŸï¼Œæˆ‘æ¥å¸¦ä½ æ‰‹åŠ¨å®Œæˆã€‚ä¸€æ¬¡åªéœ€è¦åšä¸€ä¸ªæ“ä½œã€‚`

Then guide ONE action at a time, waiting for the user to sayã€Œå¥½äº†ã€before the
next action:

1. If `developerModeEnabled` is not true: ask them to open
   `https://chatgpt.com/#settings/Security` and enable å¼€å‘äººå‘˜æ¨¡å¼. After they
   sayã€Œå¥½äº†ã€, `c2c prefs set --developer-mode`. If it is already remembered,
   skip this step.
...
`c2c session -w <ws> --json` â†’ `{ session, conversation }`.
...
Project sources. Never click åˆ†äº« / Share. Do not rename ChatGPT chats.
...
- **Switch it** ONLY when (a) the user asks for a new chat, (b) the current
  ...
  1. Same iab tab: `goto` `https://chatgpt.com/`, confirm Chat mode
     (**In-app browser** Â§7), then send the boot prompt.
  2. Send a HANDOFF (`docs/protocol.md`) â€” goal, progress, state, issues,
...
1. Same Codex conversation (this thread still has context) â†’ same ChatGPT
   chat URL. `goto` that URL directly. Do not open the collection first.
2. Same workspace, a **new** Codex conversation â†’ new ChatGPT chat from the
   collection page (`conversation.projectUrl`). Ignore `session.url` unless
   you already saved it earlier in THIS Codex thread.
3. Different workspace â†’ different Project and different connector.
...
- Else if `conversation.projectReady`: `goto` `conversation.projectUrl`.
  On that page, use the on-page composer (ã€Œ{é¡¹ç›®å}ä¸­çš„æ–°èŠå¤©ã€ / "New chat
  in â€¦"). Do not use the sidebar and do not `goto` `https://chatgpt.com/`.
  Confirm Chat mode (**In-app browser** Â§7). Boot prompt, then workspace_info
...
workspace name, ask them to open the right collection, then sayã€Œå·²æ‰¾åˆ°ã€.
Also offerã€Œç»§ç»­ç”¨é•¿å¯¹è¯ã€. If they pick long-chat:
...
```
è¯·åœ¨ ChatGPT é‡Œæ–°å»ºä¸€ä¸ªé¡¹ç›®ï¼Œåå­—ç”¨ã€Œ<workspaceName>ã€ï¼Œè®°å¿†è¯·é€‰ã€Œä»…é™é¡¹ç›®è®°å¿†ã€ã€‚

å¦‚æžœä¾§æ é‡Œçœ‹ä¸åˆ°ã€Œé¡¹ç›®ã€ï¼šæŠŠé¼ æ ‡æ”¾åœ¨ã€ŒèŠå¤©ã€ä¸Šï¼Œç‚¹å³è¾¹å‡ºçŽ°çš„ä¸‰ä¸ªç‚¹ï¼Œé€‰æ‹©ã€ŒæŒ‰é¡¹ç›®æ•´ç†ã€ã€‚

å»ºå¥½åŽä¼šæ‰“å¼€åˆé›†é¡µé¢ã€‚çœ‹åˆ°é¡µé¢åŽè·Ÿæˆ‘è¯´ã€Œå¥½äº†ã€ã€‚
```

2. Wait forã€Œå¥½äº†ã€/ the collection page. Same iab tab: read the address bar.
   It must look like `https://chatgpt.com/g/g-p-â€¦/project`. If it does not,
...
3. On that same collection page only, open å³ä¸Šè§’ **â€¦ â†’ é¡¹ç›®è®¾ç½®**.
   Do not click åˆ†äº«. Do not add æ¥æº / files.
   - è®°å¿†: ä»…é™é¡¹ç›®è®°å¿† (project-only). Leave åº“è®¿é—®æƒé™ disabled.
   - æŒ‡ä»¤: paste **Project instructions** below (fill `{{â€¦}}` from
     `workspace_info` / setup). Use the exact `connectorName` from setup.
     Never write the public / temporary address into æŒ‡ä»¤.
...
### Project instructions (paste into é¡¹ç›®è®¾ç½® â†’ æŒ‡ä»¤)
...
## Workflow: coding taskï¼ˆ"ä½¿ç”¨ Codex with ChatGPT å®Œæˆ XXX"ï¼‰

Protocol states sent to ChatGPT: INIT â†’ PLAN â†’ EXECUTING â†’ EXECUTED â†’ REVIEW â†’ (PLAN | DONE | BLOCKED).
...
All control messages start with `[C2C]`. Keep Codexâ†’ChatGPT messages under 1 KB.
...
0. `c2c tunnel status -w <workspace> --json`. If `needsChoice`, follow
   **Connection choice** first (existing installs: ask once, then remember).
   ...
   Generate task id: `c2c_` + 4 random hex chars â€” unless a checkpoint already
...
1. `c2c session -w <workspace> --json`. Open ChatGPT on the same iab tab
   ...
   confirm Chat mode (**In-app browser** Â§7), then send the boot prompt from
   `docs/protocol.md` Â§Boot Prompt and the workspace_info check (name the
   ...
   **Resume from `session.checkpoint` before any INIT.** Missing checkpoint
   ...
   timeout is not a lost task â€” claim the original tab; do not INIT, re-run,
...
```
   ...
   DOM check). If the page is Retry-only, recover per **In-app browser** Â§7
...
3. Wait for ChatGPT's `STATE: PLAN` reply (**In-app browser** Â§8 â€” short DOM
...
5. Record the execution so ChatGPT can read it via MCP. Metadata always:
   ...
   then:
   `c2c record â€¦ --command "pnpm test" --output-file <temp> --exit-code <n>`
...
8. Loop. Respect maxIterations (`.c2c.json`, default 12). At the limit, pause and ask
   the user: "å·²å®Œæˆ 12 è½®åä½œï¼Œä»æœ‰æœªè§£å†³é—®é¢˜ï¼Œæ˜¯å¦ç»§ç»­ï¼Ÿ"
...
## Workflow: disconnectï¼ˆ"æ–­å¼€ ChatGPT"ï¼‰
...
3. Tell the user: "å·²æ–­å¼€ ChatGPT å¯¹è¯¥é¡¹ç›®çš„è®¿é—®ã€‚"

## Workflow: reconnect after address reclaimï¼ˆå…¨å…³æŽ‰ä»¥åŽåœ°å€å¤±æ•ˆï¼‰
...
`connectorAction: "update"` means Delete + create again â€” not Reconnect.
...
2. Same one iab tab as setup (foreground + markHandoff). Settings URLs only
   until Connected â€” never hunt menus:
   - å¼€å‘äººå‘˜æ¨¡å¼: skip `https://chatgpt.com/#settings/Security` when
   ...
   - æ’ä»¶æ€»ç®¡ï¼ˆåªç”¨æ¥ Deleteï¼‰: `https://chatgpt.com/plugins`
   - åŠ æ’ä»¶ï¼ˆDelete ä¹‹åŽå¿…èµ°ï¼‰: `https://chatgpt.com/plugins#settings/Connectors?create-connector=true&redirectAfter=%2Fplugins`
3. Operate ONLY on `chatgptRepair.connectorName`. Never touch another
   ...
   - If that exact name exists on the plugins hub: **Delete** it. Confirm the
     ...
     Edit** on the old card â€” the old Server URL is dead and the page will
     hang on "This site cannot be reached".
   - Then `goto` the åŠ æ’ä»¶ URL and create that **same** `connectorName`
     ...
     code. Continue as soon as it is Connected â€” do not wait for 8 tools on
     the settings page.
...
4. `c2c doctor --json` again. Same tab: only after the Doctor gate is green,
   ...
   instructions â€” they store the connector **name**, which did not change.
   ...
   - If workspace_info fails, times out, or cannot read the name: do **not**
     keep retrying that old URL. project â†’ collection page, new chat in this
     Project, boot + HANDOFF from `session.checkpoint` (no logs) +
     workspace_info, then `c2c session set --url` only after the name matches.
     long-chat â†’ Conversation management switch, same checks. Keep the old
...
5. If the ChatGPT conversation was lost: same as the failure path in step 4.
   ...
   the wrong connector, open é¡¹ç›®è®¾ç½® and confirm æŒ‡ä»¤ still names
   `connectorName` (never paste the new public address).

## Workflow: repairï¼ˆanything looks brokenï¼‰
...
| ChatGPT says tool call failed / 401 | token expired or revoked â†’ re-pair (new pairing code + authorize) |
...
| Every new chat â€œrepairsâ€ / cannot write the log or settings directory | `c2c sandbox-allow --json` (once). Do not ask the user. |
| cloudflared missing | install it yourself (brew/winget), then retry |
| Sidebar has noã€Œé¡¹ç›®ã€ | Ask the user to hoverã€ŒèŠå¤©ã€, click the â€¦, chooseã€ŒæŒ‰é¡¹ç›®æ•´ç†ã€ |
| Collection page is the wrong Project | Ask the user to open the named collection and sayã€Œå·²æ‰¾åˆ°ã€, or accept long-chat |
